    ...
    for face in faces:
        ...
        for loop_idx, vert_idx in enumerate(face_vert_loc_indices):
            ...
            if map_index is None:
                ...
                if set_vcols:
                    verts_vcols_split.append(verts_vcols[vert_idx])
...
def create_mesh(new_objects,
    ...
    global verts_with_vcol
    global verts_vcols
    print("verts_vcols ", len(verts_vcols), "verts_with_vcol ", len(verts_with_vcol))
    if len(verts_vcols) > 0:
        mesh = ob.data
        vcol_layer = mesh.vertex_colors.new(name='Col', do_init=False)
        ...
        for face in me.polygons:
            ...
            for vert in verts_in_face:
                vcol = verts_vcols[vert-1]

                vcol_layer.data[i].color = vcol
    ...
    def handle_vec(line_start, context_multi_line, line_split, tag, data, vec, vec_len):
        ...
        if tag == b'v':
            if len(vec) > vec_len:
                ...
                vert_vector = mathutils.Vector((vec[0], vec[1], vec[2]))
                verts_with_vcol.append(vert_vector)
                verts_vcols.append(tuple([1.0, 1.0, 1.0, 1.0]))
